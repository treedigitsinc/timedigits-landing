import { ArrowRight, Check, X, ShieldCheck, Eye, EyeSlash, Camera, Keyboard, MouseSimple, MapPin, Lock, Heart } from "@phosphor-icons/react";
        ...
        <div className="container">
          ...
          <div className="bg-slate-50 border border-teal-500/20 rounded-3xl p-8 md:p-12 mb-20 bg-gradient-to-br from-teal-500/5 to-transparent max-w-4xl mx-auto">
            ...
            <p className="text-slate-600 leading-relaxed mb-6">
              <strong className="text-slate-900">timedigits</strong> is a time tracker that intentionally excludes
              ...
              {[
                ...
                <div key={item.label} className="flex items-center gap-2 text-sm">
                  <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-teal-500/20">
                    <Check size={14} className="text-teal-500" weight="bold" />
                  </span>
                ...
                <div key={feature.title} className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-red-500/10">
                      <feature.icon size={22} className="text-red-500" />
                    </span>
          ...
          <section className="mb-20">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-center p-4 text-slate-500 font-medium text-sm">Screenshots</th>
                    <th className="text-center p-4 text-slate-500 font-medium text-sm">Keystrokes</th>
                    <th className="text-center p-4 text-slate-500 font-medium text-sm">Mouse</th>
                    <th className="text-center p-4 text-slate-500 font-medium text-sm">Webcam</th>
                    <th className="text-center p-4 text-slate-500 font-medium text-sm">GPS</th>
                  </tr>
                </thead>
                <tbody>
                  {surveillanceComparison.map((tool) => (
                    ...
                    >
                      <td className="p-4">
                        <span className={`font-medium ${tool.tool === "timedigits" ? "text-teal-500" : "text-slate-900"}`}>
                          {tool.tool}
                        </span>
                        {tool.category === "privacy" && (
                          <span className="ml-2 text-xs bg-teal-500/20 text-teal-600 px-2 py-0.5 rounded-full">
                            Privacy-first
                          </span>
                        )}
                        {tool.category === "surveillance" && (
                          <span className="ml-2 text-xs bg-red-500/20 text-red-600 px-2 py-0.5 rounded-full">
                            Surveillance
                          </span>
                        )}
                      </td>
                      <td className="text-center p-4">
                        {tool.screenshots ? (
                          <Eye size={18} className="text-red-500 mx-auto" />
                        ) : (
                          <EyeSlash size={18} className="text-teal-500 mx-auto" />
                        )}
                      </td>
                      <td className="text-center p-4">
                        {tool.keystroke ? (
            ...
            <p className="text-center text-slate-400 text-xs mt-4">
              <EyeSlash size={14} className="inline text-teal-500" /> = No surveillance | <Eye size={14} className="inline text-red-500" /> = Has surveillance feature
            </p>
          </section>
          ...
          <section className="mb-20 max-w-3xl mx-auto">
            ...
            <div className="space-y-6">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                <h3 className="text-slate-900 font-semibold mb-2">Does timedigits take any screenshots?</h3>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                <h3 className="text-slate-900 font-semibold mb-2">What data does timedigits collect?</h3>
                <p className="text-slate-500">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                <h3 className="text-slate-900 font-semibold mb-2">Can my manager see what I'm working on?</h3>
                <p className="text-slate-500">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                <h3 className="text-slate-900 font-semibold mb-2">Is there an "activity level" or "productivity score"?</h3>
                <p className="text-slate-500">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                <h3 className="text-slate-900 font-semibold mb-2">What if my company requires surveillance software?</h3>
                <p className="text-slate-500">
          ...
          <section className="text-center max-w-2xl mx-auto">
            <div className="bg-slate-50 border border-teal-500/20 rounded-3xl p-8 md:p-12">
              <Heart size={48} className="text-teal-500 mx-auto mb-6" />
